...
function Add_URP_Reviews_From_Spreadsheet($Excel_File_Name){
    ...
    $Excel_URL = '../wp-content/plugins/ultimate-reviews/reviews-sheets/' . $Excel_File_Name;
    ...
    foreach ($Data as $Review) {
        ...
        if ($Post_ID != 0) {
            ...
            if (is_array($Category_IDs)) {wp_set_object_terms($Post_ID, $Category_IDs, 'urp-review-category');}

            $Review_Item_Count = 0;
            $Total_Score = 0;
            foreach ($Review_Cat_Columns as $Category_Name => $Value) {
                if ($Review_Cat_Types[$Category_Name] == "ReviewItem") {$Total_Score += $Value; $Review_Item_Count++;}
                update_post_meta($post_id, "EWD_URP_" . $Category_Name, $Value);
            }
...
function EWD_URP_Import_From_Spreadsheet() {
        ...
        if(!preg_match("/\.(xls.?)$/", $_FILES['Reviews_Spreadsheet']['name']) and !preg_match("/\.(csv.?)$/", $_FILES['Reviews_Spreadsheet']['name'])) {
