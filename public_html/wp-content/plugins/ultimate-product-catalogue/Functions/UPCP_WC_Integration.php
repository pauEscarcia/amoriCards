...
function UPCP_Edit_Custom_Field_To_WC($Custom_Field, $Current_Values_String) {
    ...
    if (!empty($WC_Attribute)) {
    	$wpdb->query($wpdb->prepare("UPDATE $wpdb->term_taxonomy SET taxonomy=%s WHERE taxonomy=%s", "pa_" . $Custom_Field->Field_Slug , $WC_Attribute->taxonomy));
	...
	foreach ($Attributes as $Attribute) {
		...
		if (is_array($Terms)) {
			foreach ($Terms as $Term) {
				...
				if ($Test_Current) {$wpdb->query($wpdb->prepare("UPDATE $fields_meta_table_name SET Meta_Value=%s", $Test_Current . "," . $Value));}
