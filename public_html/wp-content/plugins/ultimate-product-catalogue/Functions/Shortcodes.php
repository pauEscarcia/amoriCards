			...
			if (in_array("CustomFields", $Extra_Elements)) {
				...
				foreach ($Fields as $Field) {
					...
					if ($Custom_Fields_Blank != "Yes" or $Value->Meta_Value != "") {
						...
						elseif ($Field->Field_Type == "checkbox") {$ProductString .= "<div class='upcp-tab-title'>" . $Field->Field_Name . ":</div><span>" . str_replace(",", ", ", $Value->Meta_Value) . "</span><br>";}
						else {$ProductString .= "<div class='upcp-tab-title'>" . $Field->Field_Name . ":</div><span>" . $Value->Meta_Value . "</span><br>";}
					}
				}
				$ProductString .= "</div>";
			}
...
function BuildSidebar($category, $subcategory, $tags, $prod_name) {
	...
	if ($Full_Version == "Yes" and !empty($Product_Sort)) {
	...
	if (sizeof($Categories) > 0) {
		...
		if ($Categories_Control_Type == "Dropdown") {
			...
			$CategoriesString .= "<option value='All'>" . __("Show All", 'ultimate-product-catalogue') . "</option>";
		}
	...
	if (sizeof($SubCategories) > 0 and $Sidebar_Order != "Hierarchical") {
		...
		if ($SubCategories_Control_Type == "Dropdown") {
			...
			$SubCategoriesString .= "<option value='All'>" . __("Show All", 'ultimate-product-catalogue') . "</option>";
		}
		foreach ($SubCategories as $SubCategory) {
			if ($SubCategories_Control_Type == "Dropdown") {
				$SubCategoriesString .= "<option value='" . $SubCategory->SubCategory_ID . "' ";
	...
	if (sizeof($Tags) > 0) {
		...
		if ($Tags_Control_Type == "Dropdown") {
			...
			$TagsString .= "<option value='All'>" . __("Show All", 'ultimate-product-catalogue') . "</option>";
		}
		foreach ($Tags as $Tag) {
			if ($Tags_Control_Type == "Dropdown") {
