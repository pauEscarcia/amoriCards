...
function dahz_attr_sidebar( $attr, $context ) {
	...
	if ( !empty( $context ) ) {
		...
		$attr['aria-label'] = sprintf( esc_attr_e( '%s Sidebar', 'applique' ), dahz_get_sidebar_name( $context ) );
	}
...
function dahz_attr_menu( $attr, $context ) {
	...
	if ( !empty( $context ) ) {
		...
		$attr['aria-label'] = sprintf( esc_attr_e( '%s Menu', 'applique' ), dahz_get_menu_location_name( $context ) );
	}
...
function dahz_attr_post( $attr ) {
	...
	if ( !empty( $post ) ) {
		...
		elseif ( 'attachment' === get_post_type() && get_post_format() == 'audio' ) {

			$attr['itemtype'] = 'http://schema.org/AudioObject';
		}

		elseif ( 'attachment' === get_post_type() && get_post_format() == 'video' ) {
...
function dahz_attr_entry_published( $attr ) {
	...
	$attr['title']    = get_the_time( esc_attr__( 'l, F j, Y, g:i a', 'applique' ) );
...
function dahz_attr_comment_published( $attr ) {
	...
	$attr['title']    = get_comment_time( esc_attr__( 'l, F j, Y, g:i a', 'applique' ) );
...
function dahz_attr_comment_permalink( $attr ) {
	...
	$attr['href']      = esc_url( get_comments_link() );
	$attr['itemprop']  = 'url';
